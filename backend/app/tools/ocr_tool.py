...
def read_invoice_ocr(source: str | None, declared_no: str | None = None,
                     declared_amount: str | float | None = None) -> OCRResult | None:
    """
    结构化提取发票信息。
    ...
    try:
        if source.startswith(("http://", "https://")):
            return _read_url_ocr(source, declared_no, declared_amount)
        if source.startswith("/uploads/"):
            return _extract_file(
                Path(settings.UPLOAD_DIR) / source[len("/uploads/"):], declared_no, declared_amount
            )
...
def _read_url_ocr(url: str, declared_no: str | None,
    ...
    with tempfile.NamedTemporaryFile(suffix=suffix, delete=False) as f:
        ...
        tmp = Path(f.name)
    try:
        return _extract_file(tmp, declared_no, declared_amount)
    finally:
...
def _extract_file(p: Path, declared_no: str | None,
    ...
    if ext in IMAGE_EXTENSIONS:
        ...
        return extract_invoice(p, declared_no, declared_amount)
    return None
